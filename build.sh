...
docker login -u starudream -p "${DOCKER_TOKEN}" 1>/dev/null
docker push starudream/"${NAME}":latest 1>/dev/null

echo -e "\033[32m[ Publish Aliyun Docker Hub ]\033[0m"
docker login -u "${ALIYUN_DOCKER_USERNAME}" -p "${ALIYUN_DOCKER_TOKEN}" registry.cn-hangzhou.aliyuncs.com 1>/dev/null
